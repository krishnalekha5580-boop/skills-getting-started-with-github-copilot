document.addEventListener("DOMContentLoaded", () => {
      ...
      const participantsHtml = details.participants.length
        ? `<ul class="participants-list">${details.participants.map((email) => `
            <li class="participant-item">
              <span>${email}</span>
              <button class="participant-remove" type="button" data-activity="${name}" data-email="${email}" aria-label="Remove ${email}">×</button>
            </li>`).join("")}</ul>`
  ...
  // Handle delete icon clicks for participant removal
